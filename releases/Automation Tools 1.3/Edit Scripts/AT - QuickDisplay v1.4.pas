...
begin
  ...
  for i := slCsv.Count - 1 to slCsv.Count - 1 do begin
...
begin
  if eTxt and (slTxt.Count < 100) then begin
    slTxt.SaveToFile(ProgramPath + 'Edit Scripts\Exported.txt');
    AddMessage('Text documents exported.');
  end;
  if eCsv and (slCsv.Count < 100) then begin
    slCsv.SaveToFile(ProgramPath + 'Edit Scripts\Exported.csv');
    AddMessage('csv files exported.');
  end;
