...
var
  ...
  slFunctions, slInput, slImport, slImportValues, slImportPaths: TStringList;
  sFiles: string;
  frm: TForm;
  sb: TScrollBox;
  lbl01: TLabel;
...
begin
  pnl := TComboBox(Sender).GetParentComponent;
  // clear and disable groups, records, and path edit
  TEdit(pnl.Controls[4]).Enabled := false;
  TEdit(pnl.Controls[4]).Text := '<Path>';
  TComboBox(pnl.Controls[2]).Items.Text := '';
  TComboBox(pnl.Controls[2]).Text := '<Group>';
  TComboBox(pnl.Controls[2]).Enabled := false;
  TComboBox(pnl.Controls[3]).Items.Text := '';
  TComboBox(pnl.Controls[3]).Text := '<Record>';
  TComboBox(pnl.Controls[3]).Enabled := false;
  ...
  if Assigned(f) then begin
    ...
    TComboBox(pnl.Controls[2]).Enabled := true;
  end;
end;
...
begin
  pnl := TComboBox(Sender).GetParentComponent;
  // clear and disable records and path edit
  ...
  if TComboBox(Sender).ItemIndex = -1 then
    exit;
  // we're using a stringlist so we can access the record object later
  ...
  if TComboBox(Sender).ItemIndex > -1 then begin
    ...
    if Assigned(f) then begin
      ...
      TComboBox(pnl.Controls[3]).Enabled := true;
    end;
  end;
  slRecords.Free;
end;
...
//=========================================================================
// EnablePath: Enables the Path Edit after a record has been chosen
procedure EnablePath(Sender: TObject);
var
  pnl: TPanel;
  ed: TEdit;
begin
  ...
  for i := 0 to slFunctions.Count - 1 do begin
    ...
    if (slFunctions[i] = 'Add') then begin
      ...
      if Assigned(newelement) then begin
        if (not (s2 = '')) and (not (s2 = '<Value>')) then begin
          ...
          AddMessage('        Created element at path: '+s1+' with value: '+s2+'.');
        end
        else
          AddMessage('        Created element at path: '+s1+'.');
      end
      else
    ...
    if (slFunctions[i] = 'ElementAssign') then begin
      ...
      if Assigned(element) then begin
        ...
        if (not (s2 = '')) and (not (s2 = '<Value>')) then begin
          SetEditValue(newelement, s2);
          AddMessage('        Assigned element at path: '+s1+' with value: '+s2+'.');
        end;
      end
      else
        AddMessage('        !Couldn''t find an element at path: '+s1+'.');
    end;
    ...
    if (slFunctions[i] = 'Remove') then begin
      ...
      if Assigned(element) then begin
        if (s2 = '') or (s2 = '<Index>') then begin
          Remove(element);
          AddMessage('        Removed element at path: '+s1+'.');
        end
        else begin
          RemoveByIndex(element, s2, True);
          AddMessage('        Removed element #'+s2+' at path: '+s1+'.');
        end;
      end
      else 
        AddMessage('        !Couldn''t find an element at path: '+s1+'.');
    ...
    if (slFunctions[i] = 'Replace') then begin
      ...
      if (s = s2) or (s2 = '*') then begin
        seev(e, s1, s3);
        AddMessage('        '+s+' was replaced with '+s3+'.');
      end;
      if not Assigned(s) then AddMessage('        !Couldn''t find value at path: '+s1+'.')
    end;
    ...
    if (slFunctions[i] = 'Formula') then begin
      ...
      if Assigned(geev(e, s1)) then begin
        ...
        try
          ...
          on Exception do
            AddMessage('        !The value of the element at path: '+s1+' on is non-integer.');
        end;
      ...
      if Assigned(geev(e, s1)) then begin
        if Signature(e) = 'ARMO' then begin
          if Assigned(geev(e, 'TNAM')) then begin
            ...
            AddMessage('        Restored value at '+s1+' from '+s+' to '+geev(e, s1));
          end
    ...
    if (slFunctions[i] = 'Import') then begin
      ...
      if slImportValues.Count > 0 then begin
        for j := 0 to slImportPaths.Count - 1 do begin
          if not (slImportValues[j] = geev(e, slImportPaths[j])) then begin
