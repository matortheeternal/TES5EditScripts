...
var
  ...
  Count: longint;
  amfile: IInterface;
  done, debug, CreateRecipes, processesps, cancel: Boolean;
  frm, frm2: TForm;
...
begin
  // set debug to true to print debug messages to the message log
  debug := false;
  
  // set this to true to get prompted to use assets from esp files loaded in TES5Edit
  processesps := false;
end;
{********************************** END USER VARIABLES **********************************}
...
// nif tagging confirmation window
...
begin
  ...
  AddMessage('-----------------------------------------------------');
  AddMessage('Armor Mod Builder v1.2: Builds armor mods.');
  AddMessage('-----------------------------------------------------');
