{
  ...
  *Notes*
  ...
  - Generated NPCs are un-implemented in the game.  To put them into Skyrim you'll
    ...
    corresponding to the load order of the esp with the generated NPCs.
  - You can set user variables at line 57
}
...
var
  ...
  number, created, weight, scarchance, lastnamechance, heightlow, heighthigh, race, templatenpc: integer;
  gender, template, kwadd, fadd, pre, name, hair, brow, eyes, scar, fhair: string;
  ...
  skipheadparts, skipnaming, skiphaircolor, skipheight, skipweight, skipoutfit, skiplighting, 
  skipmorphs, skipfaceparts, skiptintlayers, female, variableheight, done, debug, processesps: boolean;
  ...
  cb01, cb02, cb03: TComboBox;
  kb01: TCheckBox;

{********************************** USER VARIABLES **********************************}  
procedure SetUserVariables;
begin
  // to skip a step set its skip boolean to true
  skipnaming := false;
  skipheadparts := false;
  skiphaircolor := false;
  skipheight := false;
  ...
  // to allow the processing of esps for NPC assets set this value to true
  processesps := true;
  
  // set editor ID prefix here
  pre := 'npcg';
  
  // debug messages, for when you want to see why something isn't working
  debug := true;
end;
{******************************** END USER VARIABLES ********************************}
...
begin
  ...
  try
    ...
    cb02.Items.Text := 'Random'#13'Male'#13'Female';
    cb02.ItemIndex := 0;
    ...
    cb03.Items.Text := 'Random'+#13+slRaces.Text;
    cb03.ItemIndex := 0;
    ...
    lbl06.Width := 90;
    lbl06.Height := 25;
    lbl06.Caption := 'Last name chance: ';
    ...
    kb01.Left := 8;
    kb01.Width := 100;
    kb01.Caption := 'Vary NPC height';
    kb01.OnClick := vnpcControl;
    ...
    lbl07.Parent := frm;
    lbl07.Top := ed04.Top;
    lbl07.Left := ed04.Left + ed04.Width + 16;
    lbl07.Width := 20;
    lbl07.Caption := 'to';
    ...
    ed05.Parent := frm;
    ed05.Top := ed04.Top;
    ed05.Left := lbl07.Left + lbl07.Width + 16;
    ed05.Width := 30;
    ed05.Text := '+5';
    ed05.Enabled := false;
    ...
    if frm.ShowModal = mrOk then begin
      ...
      if cb01.ItemIndex > 0 then 
        templatenpc := slNPCs.Objects[cb01.ItemIndex];
      gender := Lowercase(cb01.Items[cb01.ItemIndex]);
      if not SameText(cb03.Items[cb03.ItemIndex], 'Random') then race := cb03.Items[cb03.ItemIndex];
      scarchance := StrToInt(StringReplace(ed02.Text, '%', '', [rfReplaceAll]));
      lastnamechance := StrToInt(StringReplace(ed03.Text, '%', '', [rfReplaceAll]));
      if kb01.State = cbChecked then variableheight := true
...
begin
  ...
  AddMessage('-------------------------------------------------------');
  AddMessage('NPC Generator v1.4: Generates random NPCs.');
  AddMessage('-------------------------------------------------------');
  ...
  for i := 0 to FileCount - 1 do begin
    ...
    group := GroupBySignature(f, 'NPC_'); //npcs
    for j := 0 to ElementCount(group) - 1 do begin
      e := ElementByIndex(group, j);
      if not Assigned(ElementByPath(e, 'TPLT')) then 
        slNPCs.AddObject(geev(e, 'EDID'), TObject(FormID(e)));
    end;
  end;
  ...
  if number > 0 then begin
    ...
    While (created < number) do begin
      ...
      // set basic npc record values
        ...
        // set template record, if defined
        if Assigned(templatenpc) then begin
          Add(npcrecord, 'TPLT', True);
          senv(npcrecord, 'TPLT', templatenpc);
        end;
        AddMessage('        Basic NPC values set.');
      
      // decide gender of npc
      female := false;
      if (not Assigned(gender) and (Random(2) = 1)) or (Lowercase(gender) = 'female') then begin
      ...
      if not skipnaming then begin
        ...
        s := StringReplace(StringReplace(StringReplace(pre + Trim(name), ' ', '', [rfReplaceAll]), '''', '', [rfReplaceAll]), '-', '', [rfReplaceAll]);
...
begin
  ...
  AddMessage(#13#10);
  AddMessage('-------------------------------------------------------');
  AddMessage('The generator is done!');
  AddMessage(#13#10#13#10);
end;
