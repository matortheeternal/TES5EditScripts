...
var
  badfile, processed, formuladebug: boolean;
...
begin
  // set formuladebug to true to debug the formula function
  formuladebug := false;
end;
{********************************** END USER VARIABLES **********************************}
    ...
    if Assigned(f) then begin
      ...
      for i := 0 to frm.ComponentCount - 1 do begin
        if frm.Components[i].InheritsFrom(TComboBox) then begin
          if (TComboBox(frm.Components[i]).Tag = TComboBox(Sender).Tag) and (TComboBox(frm.Components[i]).Width = 75) then begin
...
begin
  ...
  for i := 0 to frm.ComponentCount - 1 do begin
    if frm.Components[i].InheritsFrom(TComboBox) then begin
      ...
      if SameText(s1, s2) and (TComboBox(frm.Components[i]).Width = 150) then begin
...
begin
  ...
  AddMessage('----------------------------------------------------------');
  AddMessage('QuickChange v2.0: For fast element modification.');
  AddMessage('----------------------------------------------------------');
...
begin
  ...
  SameText(s, 'Dragonborn.esm') or
  SameText(s, 'HearthFires.esm') or
  SameText(s, 'Skyrim.Hardcoded.keep.this.with.the.exe.and.otherwise.ignore.it.I.really.mean.it.dat') then begin
    badfile := true;
    exit;
  end;
