...
{
  QuickChange v2.3 Options Form Outline
  
  Top-Down Design:
    LEVEL 0
      Scrollbox for entries.
      Clicking + button creates a new entry.
      Clicking - button removes bottom entry if not last entry.
      Clicking OK will load entry data for later usage in an easy-to-access fashion.
      Clicking CANCEL will terminate the script.
      
    LEVEL 1
      sb := TScrollBox.Create(frm);
      sb.Align := alTop;
      sb.Height := frm.Height - 200;
    ...
    LEVEL 2
      ...
      procedure RemoveEntry:
        if lstEntries.Count > 1 then
          TPanel(lstEntries[Pred(lstEntries.Count)]).Free;
      
    LEVEL 3
      procedure FunctionManager(Sender: TObject);
        s = TComboBox(Sender).Text;
        if s = 'Add' then
          CreateAddFunction(Sender)
        else if s = 'ElementAssign' then
          CreateElementAssignFunction(Sender)
        else if s = ...
