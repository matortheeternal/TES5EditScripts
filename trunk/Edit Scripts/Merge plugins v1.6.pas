{
  ...
  *CHANGES*
  v1.6
    - Fixed asset copying procedures to be case insensitive for file folder 
      recognition and asset FormID stringreplacement.
            ...
            if Length(info2.Name) > 8 then begin
              ...
              if renumber then begin
                ...
                if (index = -1) and debug then begin
                  ...
                  dst := GetFileName(mgf)+'\'+StringReplace(info2.name, old, new, [rfReplaceAll, rfIgnoreCase]);
                  CopyFile(PChar(s + src), PChar(s + dst), True);
                  ...
                  if Length(info3.Name) > 8 then begin
                    ...
                    if renumber then begin
                      ...
                      if (index = -1) and debug then begin
                        ...
                        old := slFormIDs[index];
                        new := '00' + Copy(IntToHex64(Integer(slFormIDs.Objects[index]), 8), 3, 6);
                        dst := GetFileName(mgf)+'\'+info2.Name+'\'+StringReplace(info3.name, old, new, [rfReplaceAll, rfIgnoreCase]);
                        CopyFile(PChar(s + src), PChar(s + dst), True);
                        if debug then AddMessage('            Copying asset "'+src+'" to "'+dst+'"');
                      end;
                    end
